...
public class PrioritySchedulerTest {
    ...
    private static class PriorityDonationWorker implements Runnable {
        ...
        /* Constructor */
	PriorityDonationWorker(String name, boolean once, int priority, NamedLock locks[]) {
	    this.name = name;
	    this.once = once;
	  ...
	  KThread.currentThread().setPriority(this.priority); 
          ...
          while(amIDone == false) {
            ...
            for (int i=0; i < locks.length; i++) {
              System.out.println(this.name+" trying to acquire "+ locks[i].getName());
              System.out.println(this.name+" priority="+ this.priority);
              locks[i].acquire();
              System.out.println(this.name+" has acquired "+ locks[i].getName());
	      KThread.yield();
            }
            ...
            for (int i=locks.length-1; i >= 0; i--) {
              System.out.println(this.name+" about to release "+ locks[i].getName());
              locks[i].release();
